...
if not items:
    ...
    **안내:** 이 도구를 사용하여 교육 활동에 필요한 프롬프트를 쉽게 생성할 수 있습니다. 아래의 단계를 따라 입력해 주세요.
    ...
    3. **프롬프트 생성**: 생성된 프롬프트를 확인하고 필요에 따라 수정합니다.
    ...
    if st.button("✨ 인공지능아 프롬프트를 만들어줘") and activity_code:
        ...
        with st.spinner('🧠 프롬프트를 생성 중입니다...'):
            response = client.chat.completions.create(
                ...
                messages=[
                    {"role": "system", "content": "You are an AI that helps create system prompts for educational purposes."},
                    {"role": "user", "content": f"Create a system prompt for the topic: {input_topic}."}
                ]
            )
